...
describe("createPluginNavigation", () => {
  it("opens a workspace on the plugin's own host", () => {
    const spies = deps();
    createPluginNavigation({ serverId: "host-a", workspaceId: null }, spies).openWorkspace(
      "workspace-1",
    );
  ...
  it("hands an http(s) URL to the OS browser", async () => {
    const spies = deps();
    await createPluginNavigation({ serverId: "host-a", workspaceId: null }, spies).openExternal(
  ...
  it("refuses a scheme the browser opener would silently drop", async () => {
    const spies = deps();
    const navigation = createPluginNavigation({ serverId: "host-a", workspaceId: null }, spies);
