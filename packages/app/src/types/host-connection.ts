...
function normalizeStoredRemoteSshConnection(
  record: Record<string, unknown>,
): RemoteSshHostConnection | null {
  try {
    return createRemoteSshHostConnection({
      host: typeof record.host === "string" ? record.host : "",
      ...(typeof record.sshPort === "number" ? { sshPort: record.sshPort } : {}),
...
function normalizeStoredConnection(connection: unknown): HostConnection | null {
  ...
  if (type === "remoteSsh") {
    return normalizeStoredRemoteSshConnection(record);
  }
  if (type === "relay") {
