...
test("runInImportWorkspace creates a linked-worktree workspace under the requested project", async () => {
  ...
  const resolver = projectScopeResolver([projectRoot, linkedCwd]);
  provisioning = createWorkspaceProvisioningService({
    workspaceRegistry,
    projectRegistry,
    workspaceGitService: gitService(),
  ...
  const result = await provisioning.runInImportWorkspace(
    { cwd: linkedCwd, requestedProjectId: project.projectId },
  ...
  expect(resolver.resolve).toHaveBeenCalledWith(project.projectId, {
    force: true,
    reason: "provider-session-import",
  });
});

test("runInImportWorkspace rejects a cwd outside the requested project without registry mutation", async () => {
  const projectRoot = path.join(tmpDir, "main-project");
  ...
  const resolver = projectScopeResolver([projectRoot]);
  provisioning = createWorkspaceProvisioningService({
    workspaceRegistry,
    projectRegistry,
    workspaceGitService: gitService(),
  ...
  await expect(
    provisioning.runInImportWorkspace(
      { cwd: unrelatedCwd, requestedProjectId: project.projectId },
      async () => undefined,
    ),
  ).rejects.toThrow(`Import cwd does not belong to project: ${project.projectId}`);
  expect(await workspaceRegistry.list()).toEqual([]);
  expect(await projectRegistry.list()).toEqual([project]);
});

test("runInImportWorkspace rolls back a failed requested-project import", async () => {
  const projectRoot = path.join(tmpDir, "main-project");
  const linkedCwd = path.join(tmpDir, "linked-worktree");
  mkdirSync(projectRoot);
  mkdirSync(linkedCwd);
  ...
  provisioning = createWorkspaceProvisioningService({
    ...
    projectRegistry,
    workspaceGitService: gitService(),
    projectImportScopeResolver: projectScopeResolver([projectRoot, linkedCwd]),
    logger,
  });
...
test("runInImportWorkspace rejects simultaneous workspace and project targets", async () => {
  await expect(
    provisioning.runInImportWorkspace(
      { cwd: tmpDir, requestedWorkspaceId: "workspace", requestedProjectId: "project" },
      async () => undefined,
    ),
  ).rejects.toThrow("Import cannot target both a workspace and a project");
});
...
function projectScopeResolver(exactCwds: string[]): ProjectImportScopeResolver {
